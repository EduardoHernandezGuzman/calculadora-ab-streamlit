...
def render_wizard():
    ...
    if step >= 3 and st.session_state.get("enfoque") == "bayesiano":
        ...
        if st.session_state.session_id:
            ...
            perfect_text = (
                "Analizarás tu test A/B sin Session ID. El análisis se realizará utilizando eventos y sesiones agregados."
            )
        ...
        if st.session_state.ruta_ok:
            ...
            if is_bayes_engine(engine_key):
                extra = (
                    ...
                    else "El CSV deberá contener SessionID y conversiones por sesión (según el motor de Pablo)."
                )
            else:
                interval_txt = {
                    "centrado": "IC centrado",
    ...
    with st.sidebar:
        ...
        if is_freq_engine(engine_key):
            interval_txt = {
                ...
                "derecha": "Cola derecha (IC 95% izquierda)",
                "izquierda": "Cola izquierda (IC 95% derecha)",
            }.get(st.session_state.get("freq_interval_type"), "")
            if interval_txt:
                st.caption(f"Intervalo seleccionado: {interval_txt}")
